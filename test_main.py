"""
Aether Main™ | Spatial Assistant Workframe Reference
Version: test_main_v1.0

A clean reference implementation using the Aether framework.
This copy isolates the original working code to avoid interference
with experimental test modifications.

Structure mirrors self and binary apps, but without any test_hander modifications.
"""

# ============================================================================
# LICENSE
# ============================================================================
# Aether is a new operating system for the solar system.
# It provides an abstraction layer between the hardware and applications. It is
# designed to be portable, and it is based on the concept of a virtual machine.
# The Aether abstraction layer provides a common interface to applications.
# The applications then use Aether's services to communicate with the system.
# The Aether system is designed to be self-hosting and self-sustaining.
# It is an operating system that runs on its own hardware.
# If the user has any questions, please contact them.
# ============================================================================
...
# ========================================================================
import os
os.environ["DEARPYGUI_DONT_START_DPG_SERVER"] = "true"
...
# ============================================================================
# GLOBAL STATE (shared between pipeline components)
# ============================================================================
runtime_hands = []
runtime_objects = []
...
def main():
    ...
    hand_worker = HandPerceptionPlugin(
        broker, bus,
        hand_config.get("model_path", "models/hand_landmarker.task"),
        hand_config,
    )
    obj_worker = ObjectSpatialPlugin(
        broker, bus,
        model_config.get("weights", "yolov8n.pt"),
        model_config,
    )
        ...
        try:
            while True:
                ...
                with state_lock:
                    ...
                    current_objects = list(runtime_objects)
                processed = process_hud_overlays(raw_frame.copy(), current_hands, current_objects)
                cv2.putText(processed, "Aether Spatial Assistant", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(processed, "Press 'q' to exit", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
